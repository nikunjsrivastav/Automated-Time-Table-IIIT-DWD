...
def alloc_specific(tt, busy, rm, day, slots_to_use, f, code, typ, elec, labsd):
    for s_ in slots_to_use:
        if s_ not in slot_keys or tt.at[day, s_] != "":
            return False
    if f and f in busy[day] and set(slots_to_use) & busy[day][f]:
        return False
...
def alloc(tt, busy, rm, d, f, code, h, typ="L", elec=False, labsd=set(), ex=False, preferred_slots=None):
    if preferred_slots:
