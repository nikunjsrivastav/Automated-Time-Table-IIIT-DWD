...
def alloc_specific(tt, busy, rm, room_busy, day, slots_to_use, f, code, typ, elec, labsd, course_usage,
    ...
    if not elec:
        ...
        if key in rm:
            candidate = rm[key]
            # --- CHANGE: EXEMPT C004 FROM CONFLICT CHECKS ---
            if candidate != "C004": 
    ...
    for s_ in slots_to_use:
        if r and not elec:
            if typ == "T": v = f"{code}T ({r})"
            elif typ == "P": v = f"{code} (Lab-{r})"
            else: v = f"{code} ({r})"
        else:
    ...
    if r:
        # We still MARK C004 as busy so we can track it, 
        # but the check above allows overlaps for it.
...
def generate(courses, ws, label, seed, elective_sync, room_prefix=None, elective_room_map=None, room_busy_global=None):
        ...
        for c in course_list:
            ...
            for h, typ in [(L,"L"), (T,"T"), (P,"P")]:
                ...
                while h > 1e-9 and attempts < 400:
                    ...
                    if not placed:
                        for d in days:
                            if alloc(tt, busy, rm, room_busy, d, f, code, a, typ, is_elec_flag, labsd, True, course_usage=course_usage, class_prefix=room_prefix, rr_state=rr_state):
                                h -= a; placed = True; break
    ...
    combined_placed = assign_combined_precise_durations(tt, busy, rm, room_busy, labsd, course_usage, combined_core, rr_state=rr_state)
...
if __name__ == "__main__":
    ...
    c5_block2 = generate(c5s, ws3, "CSE-V Second Half", seed+9, sync_sem5, room_prefix='C2', elective_room_map=elective_room_map, room_busy_global=global_room_busy)
    add_csv_legend_block(ws3, "data/coursesCSE-V.csv", "CSE V", room_prefix="C2", elective_room_map=elective_room_map)
